...
func load_selected_skin():
	...
	if err == OK:
		selected_skin_index = int(config.get_value("player", "skin_index", 0))
		
var loading_index := 0

func get_next_loading_index(max_images):
	var index = loading_index
	loading_index += 1
	if loading_index >= max_images:
		loading_index = 0
	return index

func _ready():
	Global.load_selected_skin()
	reset_score()
	current_level_number = 1
